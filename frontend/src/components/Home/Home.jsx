...
function Home() {
		...
		if (spots.length > 0) {
			...
			const filteredSpots = spots.filter(spot => {
				const matchesCountry =
					filter.country === 'All' || spot.country === filter.country;
				const matchesRegion =
					filter.region === 'All' || spot.state === filter.region;
	...
	const uniqueCities = [
		...new Set(
			spots.filter(spot => spot.state === filter.region).map(spot => spot.city)
		),
	];
		...
		<div className="home-container">
			...
			{Object.keys(sortedSpots).map(region => (
				<div key={region} className="region">
					...
					<div className="spots-grid">
						{sortedSpots[region]
							.slice(
								0,
								filter.city === 'All'
									? filter.country !== 'All' && filter.region === 'All'
										? 8
										: 4
									: sortedSpots[region].length
							)
							.map(spot => (
								<Link
									to={`/spots/${spot.id}`}
					...
					{filter.city === 'All' &&
						sortedSpots[region].length >
							(filter.country !== 'All' && filter.region === 'All' ? 8 : 4) && (
